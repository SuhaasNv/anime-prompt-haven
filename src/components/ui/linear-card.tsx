...
// This project standardizes on `framer-motion` (the predecessor of the
// `motion` package). The API is identical, so the upstream `motion/react`
// import is repointed here rather than adding a second animation library.
import {
  motion,
  AnimatePresence,
  MotionConfig,
...
function DialogContainer({ children, className }: DialogContainerProps) {
  ...
  // Portaled to <body> so the dialog escapes any ancestor stacking context
  // (e.g. a transformed/animated card wrapper) and layers correctly above the
  // sticky navbar. Shared-layout `layoutId` morphs still work across the
  // portal because framer-motion tracks the React tree, not the DOM tree.
  return createPortal(
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 h-full z-[100] w-full bg-white/40 backdrop-blur-sm dark:bg-black/40"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setIsOpen(false)}
          />
          <div className={cn(`fixed inset-0 z-[100] w-fit mx-auto`, className)}>{children}</div>
        </>
      )}
    </AnimatePresence>,
    document.body,
